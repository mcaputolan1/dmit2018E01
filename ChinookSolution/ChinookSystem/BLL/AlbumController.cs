    ...
    public class AlbumController
    {
        ...
        public int Album_Add(Album item)
        {
            using(var context = new ChinookContext())
            {
                ...
                {
                    context.Albums.Add(item);   //staging step
                    context.SaveChanges();      //committed step
        ...
        private bool CheckReleaseYear(Album item)
        {
            ...
            if(!int.TryParse(item.ReleaseYear.ToString(),out releaseyear))
            {
                isValid = false;
                reasons.Add("Release year is not a number.");
            }
            if(releaseyear < 1950 || releaseyear > DateTime.Today.Year)
            {
                isValid = false;
